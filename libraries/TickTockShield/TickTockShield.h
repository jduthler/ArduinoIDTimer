...
public:
  ...
  boolean isIDMinute();
  void AlertIDTime(boolean fReset = FALSE);
  int  nGetSecond();
  void writeTime(uint8_t uHour, uint8_t uMinute, uint8_t uSecond, uint8_t uMonth, uint8_t uDay, int nYear);
  void GPSLock(int nLockAge, int nSats);
private:
  ...
  boolean fLEDOn;
  boolean fUpdateNextLoop;
  uint8_t uUpdateSecond;
